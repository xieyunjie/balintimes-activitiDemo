...
public class LoopWFCtrl {
	 ...
	 * taskService.createTaskQuery().active().taskAssignee(assignee).orderByTaskCreateTime().asc().list()
	 * */
	@Test
	public void suspendProcessInst(){ 
		ProcessInstance pi = this.runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(bizKey).singleResult();
		this.runtimeService.suspendProcessInstanceById(pi.getId());
		
		ExecutionEntity executionEntity = (ExecutionEntity) this.runtimeService.createExecutionQuery().processInstanceBusinessKey(bizKey).singleResult();
		
		System.out.println("suspend??  " + executionEntity.isSuspended()); 
	}
}
